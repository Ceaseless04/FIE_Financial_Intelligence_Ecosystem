...
def create_app(
    *,
    ...
    # MarketMind is the authority on its own name, so it is stamped here rather
    # than leaving every deployment to keep FIE_SERVICE_NAME in sync with the
    # app it happens to be running.
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        logger.info(
            ...
            embedding_model=active.settings.embedding_model,
        )
        try:
            yield
        finally:
            ...
            if owns_container:
                await active.aclose()
            logger.info("marketmind_stopped")

    app = FastAPI(
        title="MarketMind",
        version=__version__,
        description=(
            "Financial knowledge graph: entities, relationships, and cited "
            "retrieval. MarketMind records what is true and how things connect; "
            "it does not value, score, or recommend."
        ),
        lifespan=lifespan,
...
def _install_middleware(app: FastAPI) -> None:
    ...
    async def correlation_middleware(
        ...
        An inbound id is honoured so a call that began in Atlas keeps one id
        across every service it touches; otherwise one is minted here.
        """
