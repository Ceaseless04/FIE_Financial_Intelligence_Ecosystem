"""Alembic environment for CFO.ai.

CFO.ai keeps its own migration history rather than sharing the platform's. Two
...
version table is namespaced and autogenerate is scoped to the ``cfo_ai`` schema,
so a migration generated here can never propose dropping another product's
tables — it cannot see them.
"""
...
from cfo_ai.storage import models as cfo_ai_models
from fie_database.config import PostgresSettings
from fie_database.postgres import Base

SCHEMA = cfo_ai_models.SCHEMA
VERSION_TABLE = "alembic_version_cfo_ai"
...
def include_object(
    ...
    """Restrict autogenerate to CFO.ai's own schema."""
    if type_ == "table":
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # The version table lives in CFO.ai's own schema, and Alembic creates it
