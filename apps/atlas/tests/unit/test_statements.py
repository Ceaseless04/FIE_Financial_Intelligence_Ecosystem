...
A balance sheet that does not balance is not a balance sheet with a small
problem: it means the extraction is wrong, and every ratio computed from it will
be wrong in a way that looks entirely reasonable.

Money precision and period comparability moved to packages/finance with the
primitives themselves; what remains here is what Atlas owns.
"""
