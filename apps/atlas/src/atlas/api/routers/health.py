...
from atlas.api.dependencies import ContainerDep
...
async def ready(container: ContainerDep, response: Response) -> HealthReport:
    ...
    Returns 503 only when Postgres is unreachable. Degraded — the AI provider is
    down, or MarketMind is — still returns 200, because every stored filing,
    every stored statement, and every deterministic ratio and valuation
    continues to work. Taking the instance out of rotation would lose that for
    no gain.
    """
